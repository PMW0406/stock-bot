# -*- coding: utf-8 -*-
"""
v14.9 히스토리 백필 — 현행 전략(B +5%/15일, 1·2차 목표 플래그) 재시뮬레이션
 실전(GO_LIVE 2026-07-07~) 기록은 보존, 백필 구간만 교체
 A트랙: 신선(5일)신고가 -5~-1% + 이격4~8 + ret20 10~25 + 폭증제외 + 실적필터(역성장 제외/GOOD 1.5x)
        15일 or 종가 -10% 손절 · 12슬롯 · 갭 ±취소 · 서킷브레이커(10중7→5일)
 B트랙: 초대형 RSI2<10 + >MA200 + 외인20일수급(강매도 제외·매집순) · +5% 고가터치 or 15일 · 3슬롯
...
FETCH_START=(pd.Timestamp(2026,7,6)-pd.Timedelta(days=460)).strftime("%Y-%m-%d")
END="2026-07-06"   # GO_LIVE 전날까지만 시뮬 (실전 기록 보존)
GO_LIVE="2026-07-07"
PARTIAL="bf149_partial.pkl"
...
def close_flags(data, dpos_all, p, i):
    """청산시 1·2차 목표 달성 플래그 (라이브 봇 _target_flags 와 동일 정의)"""
    tk=p["code"]; H=data[tk]["H"]; e_i=dpos_all[tk][p["entry_date"]]; e=p["entry_price"]
    maxh=float(H[e_i:i+1].max())
    if p["track"]=="A":
        t1,t2=e*1.09,e*1.16
    else:
        t1=e*1.05
        hi20=float(H[max(0,e_i-20):e_i].max()) if e_i>0 else 0
        t2=min(e+0.618*(hi20-e), e*1.12) if hi20>e else 0
        if t2<=t1: t2=0
    return dict(t1=round(t1,2), t2=round(t2,2) if t2 else None,
...
if __name__=="__main__":
    ...
    for day in sim_days:
        ...
        for p in pendB:
            ...
            positions.append(dict(code=tk,name=name_map.get(tk,tk),track="B",
                entry_date=day,entry_price=round(float(o),2),
                target_price=round(float(o)*1.05,2)))
        pendB=[]
        # 1) 보유 갱신
        kept=[]
        for p in positions:
            ...
            if p["track"]=="A":
                if C[i]<=p["stop_price"]:
                    ...
                    closed.append(dict(code=tk,name=p["name"],track="A",entry_date=p["entry_date"],
                        ...
                        ret_pct=round(ret,2),reason="손절 -10%(종가)",**close_flags(data,dpos_all,p,i))); recent.append(True); continue
                if held>=15:
                    ret=(C[i]-p["entry_price"])/p["entry_price"]*100
                    closed.append(dict(code=tk,name=p["name"],track="A",entry_date=p["entry_date"],
                        entry_price=p["entry_price"],exit_date=day,exit_price=round(float(C[i]),2),
                        ret_pct=round(ret,2),reason="15일 만기",**close_flags(data,dpos_all,p,i))); recent.append(False); continue
            else:
                if H[i]>=p["target_price"]:
                    closed.append(dict(code=tk,name=p["name"],track="B",entry_date=p["entry_date"],
                        ...
                        ret_pct=5.0,reason="목표 +5%",**close_flags(data,dpos_all,p,i))); continue
                if held>=15:
                    ret=(C[i]-p["entry_price"])/p["entry_price"]*100
                    closed.append(dict(code=tk,name=p["name"],track="B",entry_date=p["entry_date"],
                        entry_price=p["entry_price"],exit_date=day,exit_price=round(float(C[i]),2),
                        ret_pct=round(ret,2),reason="15일 만기(B)",**close_flags(data,dpos_all,p,i))); continue
            kept.append(p)
        positions=kept
        # 2) 브레이커 (A 손절 기준)
    ...
    legacy=[]; live_closed=[]; live_pos=[]; old_breaker=None
    if os.path.exists("history.json"):
        old=json.load(open("history.json",encoding="utf-8"))
        if isinstance(old,dict):
            legacy=old.get("legacy",[])
            live_closed=[c for c in old.get("closed",[]) if c.get("exit_date","")>=GO_LIVE]
            live_pos=old.get("positions",[])
            old_breaker=old.get("breaker")
        else: legacy=old
    # 시뮬 종료시점 미청산 포지션은 버림 — 실전 포지션(live_pos)이 진실
    hist={"format":"v14","backfilled":f"v14.9 {sim_days[0]}~{last}",
          "positions":live_pos,"closed":closed+live_closed,"legacy":legacy,
          "breaker":old_breaker or {"pause_left":brk_pause,"reset_date":last if brk_pause>0 else ""}}
