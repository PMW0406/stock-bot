...
A_TARGET_MED      = 0.09                # A 기대목표 +9% (백테스트: 보유중 피크 중앙값 — 절반이 도달)
A_TARGET_WIN      = 0.16                # A 상단목표 +16% (승리 거래 평균 종착지)
...
def build_email(regime_on, regime_msg, sp_ret, nq_ret, sox_ret, us_date,
    ...
    if new_entries:
        rows = "".join(f"""<tr>
          ...
          <td style="padding:8px;text-align:right;color:#ffd54f;">기대 {c['close']*(1+A_TARGET_MED):,.0f}<br><span style="font-size:11px;color:#889;">잘풀리면 {c['close']*(1+A_TARGET_WIN):,.0f}</span></td>
        ...
        buy_html = f"""<div style="background:#1a2a1a;border:1px solid #00c853;padding:14px;border-radius:8px;margin-bottom:14px;">
          ...
          <p style="color:#888;font-size:12px;margin:8px 0 0;">※ 시가 갭 +{GAP_MAX:.0f}%↑ 매수 보류 · 비중: 일반 1슬롯 · 🚀 1.5슬롯<br>
          ※ 기대목표(+{A_TARGET_MED*100:.0f}%)=백테스트 피크 중앙값(절반 도달) · 잘풀리면(+{A_TARGET_WIN*100:.0f}%)=승리평균 — <b>참고선일 뿐, 매도는 {HOLD_DAYS}일/-10% 규칙이 자동 실행</b></p></div>"""
    elif regime_on:
...
<html><body style="background:#0d0d0d;color:#fff;font-family:sans-serif;padding:20px;">
  <div style="max-width:680px;margin:auto;">
    ...
    <p style="color:#555;font-size:11px;margin-top:12px;">
      ...
      5년 검증: A 전연도 플러스(CAGR 8~12% 기대) · B 승률 ~69% (+5% 목표) | 투자는 본인 판단 하에.
    </p>
  </div>
</body></html>"""
    return html
